    ...
    <header className=" bg-gradient-to-r from-purple-600 to-blue-600 p-4 pb-8 px-4 flex items-center justify-between">
      <div className="flex items-center">
        ...
        <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M10 2a6 6 0 00-6 6c0 3.866 6 10 6 10s6-6.134 6-10a6 6 0 00-6-6zm0 8a2 2 0 110-4 2 2 0 010 4z" clipRule="evenodd"></path>
        </svg>
      ...
      {/* Notification Icon */}
      <div>
        <svg className="w-6 h-6 static top-0  text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-6 lg:grid-cols-8 gap-2">
        {cuisines.map((cuisine, index) => (
          <div key={index} className="flex flex-col items-center">
            ...
            />
            <span className="mt-2 text-sm sm:text-base font-medium text-gray-700">{cuisine.name}</span>
          </div>
        ))}
      </div>
    </section>
